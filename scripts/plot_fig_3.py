...
G11 = gridspec.GridSpecFromSubplotSpec(3, 1, subplot_spec=G1[0,0])
...
G210 = gridspec.GridSpecFromSubplotSpec(3, 1, subplot_spec=G21[0,0])
...
G310 = gridspec.GridSpecFromSubplotSpec(3, 1, subplot_spec=G31[0,0])

ax_A = fig.add_subplot(G1[0,0])
ax_A.axis('off')
ax_A.text(-0.15, 1.25, 'A', transform=ax_A.transAxes, fontsize=fs+1)

ax_A1 = fig.add_subplot(G11[0,0])
ax_A2 = fig.add_subplot(G11[1,0])

ax_B = fig.add_subplot(G12[0,:])
ax_B.axis('off')
ax_B.text(-0.1, 1.25, 'B', transform=ax_B.transAxes, fontsize=fs+1)
...
ax_D11 = fig.add_subplot(G210[0,0])
ax_D12 = fig.add_subplot(G210[1,0])
...
ax_E11 = fig.add_subplot(G310[0,0])
ax_E12 = fig.add_subplot(G310[1,0])
ax_E2 = fig.add_subplot(G31[0,1])
ax_E2.set_title('Example in which the network relies more strongly on prediction', fontsize=fs, pad=15)
ax_E3 = fig.add_subplot(G31[0,2])
ax_E4 = fig.add_subplot(G3[0,1])
ax_E4.set_title('Sensory weight decreases \nwith shorter trial durations T', fontsize=fs, pad=10)
ax_E4.text(-0.35, 1.25, 'F', transform=ax_E4.transAxes, fontsize=fs+1)



# %% Plot illustration distributions

std_stims = np.array([2]) # 1, 7
mean_trails = np.array([20])
std_trails = np.array([3])

plot_illustration_input_cond(std_stims, mean_trails, std_trails, ax_1 = ax_A1, ax_2 = ax_A2, 
                             labels=True, color='k')
...
if (not os.path.exists(file_for_data_prediction_driven) or not os.path.exists(file_for_data_sensory_driven)):
    ...
    # Plot illustration distributions
    plot_illustration_input_cond(np.array([1e-1]), np.array([20]), std_trails = np.array([3]), ax_1 = ax_D11, 
                                 ax_2 = ax_D12, color='k')

    plot_illustration_input_cond(np.array([0.6]), np.array([20]), np.array([0.01]), ax_1 = ax_E11, 
                                 ax_2 = ax_E12, color='k')
...
if not os.path.exists(file_for_data):
    ...
    plot_fraction_sensory_heatmap(alpha, variability_within, variability_across, 2, xlabel='Trial variability', 
                                  ylabel='Stimulus variability', data_example = np.array([[5,0],[0,5]]), 
                                  data_text = ['C', 'D'], ax = ax_D4)
...
if (not os.path.exists(file_for_data_control) or not os.path.exists(file_for_data_shorter)):
    ...
    plot_weight_over_trial(weight_control, weight_shorter, n_trials, ax = ax_E4, lw=0.5,
                            leg_text=['T', 'T/5']) # ['trial duration', 'trial duration / 5']
