...
figsize=(15/inch,8/inch)
fig = plt.figure(figsize=figsize)

G = gridspec.GridSpec(2, 1, figure=fig, height_ratios=[1.5,1], hspace=1.5)
...
ax_CDE = fig.add_subplot(G2[0,:])
ax_CDE.axis('off')
ax_CDE.set_title('Impact of (un-)correlated changes in the weights and network sparsity on the estimation error', fontsize=fs, pad=20)

ax_D = fig.add_subplot(G2[0,1])
plt.setp(ax_D.get_yticklabels(), visible=False)
ax_D.text(-0.2, 1.1, 'D', transform=ax_D.transAxes, fontsize=fs+1)
...
ax_E.text(-0.2, 1.1, 'E', transform=ax_E.transAxes, fontsize=fs+1)
