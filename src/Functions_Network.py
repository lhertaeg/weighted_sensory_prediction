...
class Stimulation_new:
    def __init__(self, stimuli, stimuli_std, SD_individual = dtype(1), prediction=None):
        
        self.SD = SD_individual
        self.stim_visual = dtype(stimuli)
        self.stim_visual_std: dtype = dtype(stimuli_std)
        if prediction is not None:
            self.stim_motor = prediction
        else:
            self.stim_motor = stimuli
...
def RunStaticNetwork_new(NeuPar, NetPar, InPar, StimPar, SimPar, RatePar, folder: str, fln: str = ''):
    ...
    stim_visual_std = iter(StimPar.stim_visual_std)
    ...
    V = np.zeros(num_time_steps, dtype=dtype)
    M = np.zeros(num_time_steps, dtype=dtype)
    ...
    for s in range(NStim):
        ...
        V_mean = next(stim_visual)
        M_mean = next(stim_motor)
        #V_std = next(stim_visual_std)
        
        V[:] = V_mean #np.random.normal(V_mean, V_std, size=num_time_steps)
        #M = sensory_input_euler(V, 100)
        M[:] = M_mean
        ...
        for tstep in range(num_time_steps):
            ...
            stim_IN[:] = V[tstep] * neurons_visual[nE:] + M[tstep] * neurons_motor[nE:] + noise_soma[nE:,tstep] + inp_ext_soma[nE:]
            StimSoma_P[:], StimSoma_S[:], StimSoma_V[:] = np.split(stim_IN, ind_break)
            
            StimSoma_E[:] = V[tstep] * neurons_visual[:nE] + noise_soma[:nE,tstep] + inp_ext_soma[:nE]
            StimDend[:] = M[tstep] * neurons_motor[:nE] + noise_dend[:,tstep] + inp_ext_dend
        ...
        # save data - here at the end of each stimulus
