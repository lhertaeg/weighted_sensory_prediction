...
%
\begin{align}
r_\mathrm{nE} = [h_\mathrm{nE}]_+ \ \mbox{ with }\ \tau_E\ \frac{dh_\mathrm{nE}}{dt} &= - h_\mathrm{nE} + w_\mathrm{nEnD}\cdot  r_\mathrm{nD}  -  w_\mathrm{nEP_1}\cdot r_\mathrm{P_1}  -  w_\mathrm{nEP_2}\cdot r_\mathrm{P_2} + I_\mathrm{nE}, \nonumber\\
r_\mathrm{pE} = [h_\mathrm{pE}]_+ \ \mbox{ with }\ \tau_E\ \frac{dh_\mathrm{pE}}{dt} &= - h_\mathrm{pE} + w_\mathrm{pEpD}\cdot  r_\mathrm{pD}  -  w_\mathrm{pEP_1}\cdot r_\mathrm{P_1}  -  w_\mathrm{pEP_2}\cdot r_\mathrm{P_2} + I_\mathrm{pE}
\end{align}
%
...
%
\begin{align}
r_\mathrm{nE} = [h_\mathrm{nD}]_+ \ \mbox{ with }\ \tau_E\ \frac{dh_\mathrm{nD}}{dt} &= - h_\mathrm{nD} +  w_\mathrm{nDnE}\cdot r_\mathrm{nE} +  w_\mathrm{nDpE}\cdot r_\mathrm{pE}  - w_\mathrm{nDS}\cdot r_\mathrm{S} + I_\mathrm{nD}, \nonumber\\
r_\mathrm{pD} = [h_\mathrm{pD}]_+ \ \mbox{ with }\ \tau_E\ \frac{dh_\mathrm{pD}}{dt} &= - h_\mathrm{pD} +  w_\mathrm{pDnE}\cdot r_\mathrm{pE} +  w_\mathrm{pDpE}\cdot r_\mathrm{pE}  - w_\mathrm{pDS}\cdot r_\mathrm{S} + I_\mathrm{pD}
\end{align}
%
where the weights $w_{\mathrm{nDnE}}$, $w_{\mathrm{nDpE}}$, $w_{\mathrm{pDnE}}$ and $w_{\mathrm{pDpE}}$ denote the recurrent excitatory connections between PCs, including backpropagating activity from the soma to the dendrites. $w_{\mathrm{nDS}}$ and $w_{\mathrm{pDS}}$ represent the strength of dendritic inhibition from SOM neurons. The overall inputs $I_\mathrm{nD}$ and $I_\mathrm{pD}$ comprise fixed, external background inputs and feedback predictions (see ``Inputs" below). We assume that any excess of inhibition in a dendrite does not affect the soma, that is, the dendritic compartment is rectified at zero. 

Similarly, the firing rate dynamics of each interneuron is modeled by a rectified, linear differential equation,
%
\begin{align}
\label{eq:RateEqINs}
r_\mathrm{X} =& [h_\mathrm{X}]_+ \ \mbox{ with }\ \tau_I\ \frac{dh_\mathrm{X}}{dt} = -h_\mathrm{X} + I_{\mathrm{X}} + w_\mathrm{XE}\cdot r_\mathrm{E} - w_\mathrm{XP_1}\cdot r_\mathrm{P_1} - w_\mathrm{XP_2}\cdot r_\mathrm{P_2}  - w_\mathrm{XS}\cdot r_\mathrm{S} -  w_\mathrm{XV}\cdot r_\mathrm{V},
\end{align}
%
where $r_\mathrm{X}$ denotes the firing rate of interneuron type $X$, and the weight $w_\mathrm{XY}$ denotes the strength of connection between the presynaptic neuron $Y$ and the postsynaptic neuron $X$ ($X, Y\in \lbrace P_1, P_2, S, V\rbrace$). The rate time constant $\tau_I$ was chosen to resemble a fast GABA\textsubscript{A} time constant, and set to 2 ms for all interneuron types included. The overall input $I_\mathrm{X}$ comprises fixed, external background inputs, as well as feedforward sensory inputs and feedback predictions (see ``Inputs" below).
...
%
\begin{align}
\tau_M \cdot \frac{dr_\mathrm{M}}{dt} = w_\mathrm{MpE} \cdot r_\mathrm{pE} - w_\mathrm{MnE} \cdot r_\mathrm{nE}.
\end{align}
%
$w_\mathrm{MpE}$ denotes the connection strength between the pPE neuron and the memory neuron, and  $w_\mathrm{MnE}$ denotes the connection strength between the nPE neuron and the memory neuron. The time constant XXX.

The dynamics of the variance neuron obeys a non-linear differential equation with leak term,
%
\begin{align}
\tau_V \cdot \frac{dr_\mathrm{V}}{dt} = -r_\mathrm{V} + (w_\mathrm{VpE} \cdot r_\mathrm{pE} + w_\mathrm{VnE} \cdot r_\mathrm{nE})^2.
\end{align}
%
The weight $w_\mathrm{VpE}$ represents the connection strength between the pPE neuron and the variance neuron, while  $w_\mathrm{VnE}$ denotes the connection strength between the nPE neuron and the variance neuron.  XXXX contiue here XXXX We chose a squared activation function ... but in xxx we also use a xxx this then is not the variance but xxx. the time constant was chosen to be xxx.
...
For population model for the lower PE circuit, xxx.
